...
def CreateLabelsAndAttr(network, prior=.5, conds=[[0.3, .4, .35], [.85, .65, .75]]):
	labels = {}
	attrs = {}
	for vertex in network:
		labels[vertex] = int(random.random() < prior)
		attrs[vertex] = map(lambda x: int(random.random() < x), conds[labels[vertex]])
...
class SimpleBernoulliAR:
	...
	def set_accept_probs(self, ap=[1.0, .75, 1.0]):
...
def CreateAGMSamples(size, verbose_check=True, show_dd=False):
	...
	if verbose_check:
		print('FCL Graph Label Correlation', ComputeLabelCorrelation(fcl_sample, labels))
		print('AGM Graph Label Correlation', ComputeLabelCorrelation(agm_sample, labels))
